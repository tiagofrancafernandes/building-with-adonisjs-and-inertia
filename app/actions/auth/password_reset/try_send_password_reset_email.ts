...
export default class TrySendPasswordResetEmail {
    static async handle({ email }: Params) {
        ...
        const resetLink = router.builder().prefixUrl(env.get('APP_URL')).params({ value: encryptedValue }).make('forgot_password.reset');

        await mail.sendLater((message) => {
            message.subject('Reset Your PlotMyCourse Password').to(user.email).htmlView('emails/forgot_password', {
                user,
                resetLink,
            });
        });
