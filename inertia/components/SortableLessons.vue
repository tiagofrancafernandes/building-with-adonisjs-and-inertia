...
function onSubmit() {
    ...
    const action = form.transform(({ publishAtDate, publishAtTime, ...data }) => ({
        ...data,
        publishAt: publishAtDate && publishAtTime ? DateTime.fromISO([publishAtDate, publishAtTime].join('T')).toUTC() : null,
    }));
...
<template>
    ...
    <FormDialog
        ...
        @update="onSubmit"
    >
        <FormInput ref="dialogFocusEl" label="Name" v-model="form.name" :error="form.errors.name" placeholder="My Cool Lesson" />
    ...
    <ConfirmDestroyDialog v-model:open="destroy.isOpen" title="Delete Lesson?" :action-href="`/lessons/${destroy.resource?.id}`">
        Are you sure you'd like to delete your
        <strong>{{ destroy.resource?.name }}</strong>
        lesson? All this lesson's data will be deleted forever.
